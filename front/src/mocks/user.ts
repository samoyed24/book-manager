...
import mockResponse, { ListMockData, mockPagedTableData } from '@/mocks/wrapper';
import Mock from 'mockjs';
import { UserFetchResult, UserRequestParams } from 'types/user';

export default [
  {
    url: '/api/user/getUsers',
    method: 'get',
    response: ({ query }: { query: UserRequestParams }) => {
      const data: ListMockData<UserFetchResult> = Mock.mock({
        'list|10': [
          {
            name: '@cword(2, 3)',
            username: '@word(5, 10)',
