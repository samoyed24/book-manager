...
import { TableFetchResult } from '../index.vue'

export interface PagedTableReturnData<T extends TableFetchResult> {
    content: T[]
    empty: boolean
    first: boolean
    last: boolean
...
export interface TableColumnProps<T extends TableData = TableData> {
    label?: string
    prop: keyof T | (string & {})
    width?: number | string
...
export interface CustomTableProps<T extends TableData = TableData> {
    tableColumnProps: TableColumnProps<T>[]
    tableData: TableData[]
    indexWidth?: string
    height?: number
    loading: boolean
